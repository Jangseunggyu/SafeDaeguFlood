...
# 상위 10개 출력
print(dens_sorted[['행정구', '읍면동', '인구(명)', '면적(㎢)', '인구밀도(명/㎢)']].head(10))
...
# 2. 특정 행정구 선택 후 동별 인구밀도 보기 (예: 중구)
target_admin = '중구'
dong_dens = dens[dens['행정구'] == target_admin].sort_values(by='인구밀도(명/㎢)', ascending=False)

print(f"▶ {target_admin} 행정구 읍면동별 인구밀도")
print(dong_dens[['읍면동', '인구밀도(명/㎢)']])

# 3. 시각화
plt.figure(figsize=(8, 5))
plt.bar(dong_dens['읍면동'], dong_dens['인구밀도(명/㎢)'], color='skyblue')
plt.xlabel('읍면동')
plt.ylabel('인구밀도 (명/㎢)')
plt.title(f'{target_admin} 행정구 읍면동별 인구밀도')
plt.xticks(rotation=45)
plt.tight_layout()
plt.show()
