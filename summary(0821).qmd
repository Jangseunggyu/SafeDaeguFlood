...
# =========================
stations = [
    (828, "달성",    35 + 41/60, 128 + 25/60, "KMA 연보"),
    (845, "대구북구", 35 + 54/60, 128 + 35/60, "KMA 연보"),
    (846, "대구서구", 35 + 51/60, 128 + 31/60, "KMA 연보"),
    (860, "신암",    35 + 53/60, 128 + 37/60, "KMA 연보"),
    (991, "옥포",    35.794916, 128.440565, "대체좌표"),
    (992, "하빈",    35.900780, 128.446059, "대체좌표"),
]
df_stn = pd.DataFrame(stations, columns=["지점","지점명","lat","lon","좌표출처"])
...
districts = [
    ...
    ("달서구", 35.82569, 128.52403),
    ("달성군", 35.77467, 128.42955),
    ("군위군", 36.16995, 128.64705),  # 2023-07-01 편입
]
df_ctr = pd.DataFrame(districts, columns=["구군","lat","lon"])
...
def idw_estimate(target_lat, target_lon, stations_df, values_series, p=2, k=4):
    ...
    for _, r in stations_df.iterrows():
        ...
        if pd.notnull(val):
            d = haversine_km(target_lat, target_lon, slat, slon)
            rows.append((sid, max(d,1e-6), val))  # 0거리 보호
    if not rows:
        return float("nan")
    rows.sort(key=lambda x: x[1])
    rows = rows[:k]
    w = [1/(d**p) for _, d, _ in rows]
    wsum = sum(w)
    return sum(wi*val for wi, (_, _, val) in zip(w, rows)) / wsum if wsum>0 else float("nan")

# =========================
# 4) 일×지점 → 구군별 IDW 추정 df_out
# =========================
pv = rain.pivot_table(index="일시", columns="지점", values="일강수량", aggfunc="mean")
rows = []
for the_date, row in pv.iterrows():
    for _, g in df_ctr.iterrows():
        est = idw_estimate(g["lat"], g["lon"], df_stn[["지점","lat","lon"]], row, p=2, k=4)
        rows.append([the_date, g["구군"], est])
df_out = pd.DataFrame(rows, columns=["일시","구군","추정_일강수량(mm)"]).sort_values(["일시","구군"]).reset_index(drop=True)

# =========================
# 5) 위험지표 & 점수화 (정규화+로그)
# =========================
def compute_rain_risk_fair(
    df_in: pd.DataFrame,
    c_date="일시", c_region="구군", c_rain="추정_일강수량(mm)",
    start=None, end=None, scale="minmax"
):
    # --- 전처리 ---
    df = df_in.rename(columns={c_date:"일시", c_region:"구군", c_rain:"rain"}).copy()
    df["일시"] = pd.to_datetime(df["일시"])
    if start is None: start = df["일시"].min()
    if end   is None: end   = df["일시"].max()
    df = df[(df["일시"]>=pd.Timestamp(start)) & (df["일시"]<=pd.Timestamp(end))]
    wide = (df.pivot(index="일시", columns="구군", values="rain")
              .asfreq("D")     # 결측 날짜 포함
              .fillna(0.0))
    # --- 지표 계산 ---
    # Frequency: 임계값별 일수
    ...
    freq_parts = {}
    for t in ths:
        freq_parts[f"heavy{t}_days"] = (wide >= t).sum(axis=0)

    # Intensity: 1/3/5일 최대 누적 (로그 완화)
    max_1d = wide.max(axis=0)
    max_3d = wide.rolling(3, min_periods=1).sum().max(axis=0)
    max_5d = wide.rolling(5, min_periods=1).sum().max(axis=0)

    # Saturation(API): k=0.85, 최대값 (로그 완화)
    def api_df(m, k=0.85):
        arr = m.to_numpy(copy=True)
        out = np.zeros_like(arr, float)
        acc = np.zeros(arr.shape[1], float)
        for i in range(arr.shape[0]):
            acc = k*acc + arr[i]
            out[i] = acc
        return pd.DataFrame(out, index=m.index, columns=m.columns)
    API_max = api_df(wide, 0.85).max(axis=0)

    # Seasonality: 장마(6/15~8/15) 합
    def in_window(idx, m1d, m2d):
        md = (idx.month*100 + idx.day)
        a = m1d[0]*100 + m1d[1]; b = m2d[0]*100 + m2d[1]
        return (md>=a) & (md<=b)
    monsoon_total = wide[in_window(wide.index, (6,1), (9,30))].sum(axis=0)

    # Concentration: 상위 10일 집중도
    def topk_share(series, k=10):
        tot = series.sum()
        if tot <= 0: return 0.0
        arr = np.sort(series.values)[-k:] if len(series)>=k else series.values
        return float(arr.sum()/tot)
    top10_share = wide.apply(topk_share, axis=0)

    # --- 변환(로그/루트) + 정규화 ---
    def safe_log1p(s):  # 분포 완화
        return np.log1p(s.clip(lower=0))

    def minmax01(s):   # 0~1
        s = s.astype(float)
        vmin, vmax = s.min(), s.max()
        if vmax == vmin:  # 모두 같으면 0.5
            return pd.Series(0.5, index=s.index)
        return (s - vmin) / (vmax - vmin)

    # Intensity 30% : log1p(max_1d/3d/5d) 가중 평균
    I = (0.4 * minmax01(safe_log1p(max_1d))
       + 0.4 * minmax01(safe_log1p(max_3d))
       + 0.2 * minmax01(safe_log1p(max_5d)))

    # Frequency 30% : 임계값 낮을수록 가중치 낮게 → 30/50/80/100 = 0.2/0.3/0.3/0.2
    F = (0.2 * minmax01(freq_parts["heavy30_days"])
       + 0.3 * minmax01(freq_parts["heavy50_days"])
       + 0.3 * minmax01(freq_parts["heavy80_days"])
       + 0.2 * minmax01(freq_parts["heavy100_days"]))

    # Saturation 20% : log1p(API_max)
    S = minmax01(safe_log1p(API_max))

    # Seasonality 10% : monsoon_total
    SE = minmax01(monsoon_total)

    # Concentration 10% : top10_share
    C = minmax01(top10_share)

    # 최종 점수(0~100)
    Risk = (0.30*I + 0.30*F + 0.20*S + 0.10*SE + 0.10*C) * 100.0

    # 요약/메트릭 테이블
    metrics = pd.DataFrame({
        "max_1d": max_1d, "max_3d": max_3d, "max_5d": max_5d,
        "heavy30_days": freq_parts["heavy30_days"],
        "heavy50_days": freq_parts["heavy50_days"],
        "heavy80_days": freq_parts["heavy80_days"],
        "heavy100_days": freq_parts["heavy100_days"],
        "API_max": API_max,
        "monsoon_total": monsoon_total,
        "top10_share": top10_share,
        "RiskScore": Risk.round(1)
    }).sort_values("RiskScore", ascending=False)

    summary = metrics[[
        "RiskScore","max_3d","max_1d","heavy30_days","heavy50_days",
        "heavy80_days","heavy100_days","API_max","monsoon_total","top10_share"
    ]].round(2)
...
# 6) 실행: 결과만 출력(파일 저장 X)
# =========================
summary, metrics = compute_rain_risk_fair(df_out)

# print("📌 위험도 요약 (RiskScore 기준 정렬)")
# print(summary)
...
score_0601_0930 = metrics['RiskScore'].reset_index()
score_0601_0930
# score_0601_0930.to_csv("대구 행정구별 강수량 스코어_0601_0930.csv", index=False, encoding="utf-8-sig")



# 행정구 내 읍면동 삽입

df = pd.DataFrame(score_0601_0930)
...
for _, row in df.iterrows():
    gu = row["구군"]
    risk = row["RiskScore"]
    for dong in mapping[gu]:
        expanded_rows.append({"구군": gu, "RiskScore": risk, "읍면동": dong})

# 최종 DataFrame
df_expanded = pd.DataFrame(expanded_rows)
# df_expanded.to_csv("읍면동 강수량 리스크 스코어_0601_0930.csv", index=False, encoding="utf-8-sig")
...
pd.read_csv('./data/읍면동 강수량 리스크 스코어_0601_0930.csv')
...
# oldest_pump.to_csv("펌프 리스크 스코어.csv", index=False, encoding="utf-8-sig")
# ======================================================================
...
# pump_details.to_csv("읍면동 펌프장 리스트.csv", index=False, encoding="utf-8-sig")
...
def create_simple_elevation_map(csv_file, geojson_file):
    """
    간단하고 확실한 Folium 고도 지도
    """
    ...
    try:
        ...
        print(f"✅ CSV 로드: {len(df_main)}개 행정동")
        
        with open(geojson_file, 'r', encoding='utf-8') as f:
            geojson_data = json.load(f)
        
        gdf_admin = gpd.GeoDataFrame.from_features(geojson_data['features'])
        gdf_admin = gdf_admin.set_crs('EPSG:4326')
        print(f"✅ GeoJSON 로드: {len(gdf_admin)}개 구역")
    ...
    for idx, admin_row in gdf_admin.iterrows():
        admin_name = admin_row['adm_nm']
        
        # 동 이름 추출 (마지막 단어)
        dong_name = admin_name.split()[-1] if admin_name else "Unknown"
        
        # CSV에서 매칭
        matched = df_main[df_main['행정동'].str.contains(dong_name, na=False)]
        
        if len(matched) > 0:
            elevation = float(matched.iloc[0].get('최저_고도', 100.0))
            population = int(matched.iloc[0].get('등록인구', 0))
        else:
            elevation = 100.0  # 기본값
            population = 0
    ...
    print(f"✅ 데이터 병합: {len(gdf)}개 지역")
    
    # 3. 인근 지역 분석
    print("📊 인근 지역 고도 분석 중...")
    
    # 좌표 배열 생성
    coords = np.array([[row['lon'], row['lat']] for _, row in gdf.iterrows()])
    
    # KNN으로 인근 5개 지역 찾기
    nbrs = NearestNeighbors(n_neighbors=6).fit(coords)  # 자신 포함 6개
    ...
    for i, row in gdf.iterrows():
        ...
        # 인근 지역들의 고도 (자신 제외)
        neighbor_indices = indices[i][1:]  # 첫 번째는 자기 자신
        neighbor_elevations = [gdf.iloc[j]['elevation'] for j in neighbor_indices]
        neighbor_avg = np.mean(neighbor_elevations)
        
        # 고도 차이 계산
        diff = current_elevation - neighbor_avg
        
        neighbor_averages.append(neighbor_avg)
        elevation_diffs.append(diff)
    
    gdf['neighbor_avg'] = neighbor_averages
    gdf['elevation_diff'] = elevation_diffs
    
    # 4. 상대적 낮음 정도 계산
    min_diff = min(elevation_diffs)
    max_diff = max(elevation_diffs)
    
    relative_scores = []
    for diff in elevation_diffs:
        if diff <= 0:  # 낮은 경우
            score = abs(diff) / abs(min_diff) if min_diff < 0 else 0
        else:  # 높은 경우
            score = 0
        relative_scores.append(score)
    
    gdf['lowness_score'] = relative_scores
    
    print(f"📈 고도 차이 범위: {min_diff:.1f}m ~ {max_diff:.1f}m")
    ...
    def get_opacity(lowness_score):
        """낮음 점수에 따른 투명도"""
        return min(0.9, 0.3 + lowness_score * 0.6)
    
    # 7. 지역별 폴리곤 추가
    for _, row in gdf.iterrows():
        diff = row['elevation_diff']
        score = row['lowness_score']
        
        # 팝업 내용
        popup_text = f"""
        <div style="font-family: Arial; width: 200px;">
            <h4 style="margin: 0; color: #2C3E50;">{row['dong_name']}</h4>
            <hr style="margin: 5px 0;">
            <p><b>현재 고도:</b> {row['elevation']:.1f}m</p>
            <p><b>인근 평균:</b> {row['neighbor_avg']:.1f}m</p>
            <p><b>고도 차이:</b> <span style="color: {'red' if diff < 0 else 'blue'};">
               {diff:+.1f}m</span></p>
            <p><b>등록인구:</b> {row['population']:,}명</p>
            <p><b>상태:</b> <span style="color: {'red' if diff < 0 else 'green'};">
               {'인근보다 낮음' if diff < 0 else '인근보다 높음'}</span></p>
        </div>
        """
        ...
        folium.GeoJson(
            row.geometry,
            style_function=lambda x, color=get_color(diff), opacity=get_opacity(score): {
                'fillColor': color,
                'color': 'black',
                'weight': 1,
                'fillOpacity': opacity
            ...
            tooltip=f"{row['dong_name']}: {diff:+.1f}m"
        ).add_to(m)
    
    # 8. 위험 지역 마커 추가
    high_risk = gdf[gdf['lowness_score'] > 0.5]
    
    if len(high_risk) > 0:
        print(f"⚠️ 고위험 지역: {len(high_risk)}곳")
        
        for _, row in high_risk.iterrows():
            folium.Marker(
                location=[row['lat'], row['lon']],
                popup=f"<b>⚠️ 주의 필요</b><br>{row['dong_name']}<br>낮음 점수: {row['lowness_score']:.2f}",
    ...
    <h4 style="margin: 0 0 10px 0;">🗺️ 고도차 범례</h4>
    <p><span style="color:#8B0000;">🔴 < -15m</span> 매우 낮음</p>
    <p><span style="color:#DC143C;">🔴 -15 ~ -10m</span> 낮음</p>
    <p><span style="color:#FF6347;">🟠 -5 ~ 0m</span> 약간 낮음</p>
    <p><span style="color:#87CEEB;">🔵 0 ~ 5m</span> 약간 높음</p>
    <p><span style="color:#000080;">🔵 > 10m</span> 매우 높음</p>
    <hr>
    <p style="font-size:10px;">진할수록 인근보다 상대적으로 더 낮음</p>
    </div>
    '''
    m.get_root().html.add_child(folium.Element(legend_html))
    
    # 10. 저장
    output_file = "daegu_elevation_simple.html"
    m.save(output_file)
    print(f"✅ 지도 저장: {output_file}")
    
    # 11. 통계 출력
    print(f"\n📊 분석 결과:")
    print(f"• 분석 지역: {len(gdf)}개")
    print(f"• 인근보다 낮은 지역: {len(gdf[gdf['elevation_diff'] < 0])}개")
    print(f"• 고위험 지역: {len(high_risk)}개")
    print(f"• 평균 고도: {gdf['elevation'].mean():.1f}m")
    
    # 가장 낮은 지역 TOP 5
    lowest = gdf.nsmallest(5, 'elevation_diff')
    print(f"\n🔻 인근보다 가장 낮은 지역:")
    for i, (_, row) in enumerate(lowest.iterrows(), 1):
        print(f"  {i}. {row['dong_name']}: {row['elevation_diff']:.1f}m")
    
    # 결과 데이터 저장
    result_df = gdf.drop('geometry', axis=1)
    # result_df.to_csv("elevation_analysis_simple.csv", index=False, encoding='utf-8-sig')
    # print(f"✅ 분석 데이터 저장: elevation_analysis_simple.csv")
    
    return m

# 실행
if __name__ == "__main__":
    csv_file = "./data/df1_찐막_with_coordinates.csv"
    geojson_file = "./data/hangjeongdong_대구광역시.geojson"
    
    try:
        map_result = create_simple_elevation_map(csv_file, geojson_file)
        
        if map_result:
            print(f"\n🎉 완료!")
            print(f"📁 'daegu_elevation_simple.html' 파일을 브라우저에서 열어보세요!")
            print(f"📋 사용법:")
            print(f"  • 지역 클릭 → 상세 정보")
            print(f"  • 빨간색 = 인근보다 낮음")
            print(f"  • 파란색 = 인근보다 높음")
        ...
        print(f"❌ 파일 없음: {e}")
        print("필요한 파일:")
        print("  • df1_찐막_with_coordinates.csv")
        print("  • hangjeongdong_대구광역시.geojson")
    except Exception as e:
        print(f"❌ 오류: {e}")
        import traceback
        traceback.print_exc()
...
elevation = pd.read_csv('./data/elevation_analysis_simple.csv')

# 전체 읍면동 고도: elevation(고도), neighbor_avg(주변고도), elevation_diff(주변대비 고도 차이값)
elevation[['dong_name','elevation','neighbor_avg','elevation_diff','lowness_score']]



# 주변보다 낮은 고도 읍면동
elevation[elevation['elevation_diff'] < 0][['dong_name','elevation','neighbor_avg','elevation_diff','lowness_score']]
